...
class Grid:
    ...
    def __init__(self, target: ItemId, *, cols: int = 1, rows: int = 1, spacing: Point = (0, 0),
                 padding: Point = (0, 0)):
        """Args:
    ...
    def pack(
        ...
        if anchor not in self.ANCHORS:
            raise ValueError(
                f"Accepted `anchor` values; 'n', 'ne', 'e', 'se', 's', 'sw', 'w', 'nw', 'c' (got {anchor!r}).")
    ...
    ANCHORS = {
        ...
        "e": lambda i_wt, i_ht, c_x, c_y, c_wt, c_ht: (int((c_wt - i_wt) + c_x), int((c_ht - i_ht) / 2 + c_y)),
        # center y
        "se": lambda i_wt, i_ht, c_x, c_y, c_wt, c_ht: (int((c_wt - i_wt) + c_x), int((c_ht - i_ht) + c_y)),
        "s": lambda i_wt, i_ht, c_x, c_y, c_wt, c_ht: (int((c_wt - i_wt) / 2 + c_x), int((c_ht - i_ht) + c_y)),
        # center x
        ...
        "c": lambda i_wt, i_ht, c_x, c_y, c_wt, c_ht: (int((c_wt - i_wt) / 2 + c_x), int((c_ht - i_ht) / 2 + c_y)),
        # center x & y
    }
